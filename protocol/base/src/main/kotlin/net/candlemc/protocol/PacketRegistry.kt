...
class PacketRegistry(
     ...
     */
    internal val localRegistry: ConcurrentHashMap<PacketKey, Packet> = ConcurrentHashMap()
     ...
     *
     * @param packet the Packet instance to be registered.
     ...
     */
    fun registerPacket(packet: Packet): Packet {
        val key = PacketKey(packet.connectionState, packet.id)
        ...
        eventBus.publish(PacketPreRegisterEvent(this, key)).thenRun {
            localRegistry[key] = packet
            updateCacheOnRegistration(key)
            eventBus.publish(PacketPostRegisterEvent(this, key, packet))
     ...
     */
    fun getPacket(key: PacketKey): Packet? {
